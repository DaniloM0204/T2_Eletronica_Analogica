...
O JFET é um dispositivo semicondutor de três terminais: Gate (G), Source (S) e Drain (D). Seu funcionamento baseia-se no controle do fluxo de portadores de carga através de um canal semicondutor (de tipo N ou P), que é modulado pela tensão aplicada entre o gate e a fonte. \cite{blikai_jfet_2025}
...
Quando uma tensão \(V_{DS}\) é aplicada entre o dreno e a fonte, uma corrente \(I_D\) começa a fluir através do canal, composta por elétrons (no JFET de canal N) ou lacunas (no JFET de canal P). A magnitude dessa corrente depende da largura efetiva do canal condutor. \cite{electronics_tutorials_jfet_2025}
...
Para o transistor JFET pode-se distinguir três regiões principais de operação: \cite{eletronica_analogica2015_jfet_2025}
...
O JFET é construído a partir de um cristal semicondutor (geralmente silício) no qual se define um canal condutor de tipo N ou tipo P. O canal é cercado por regiões de dopagem oposta, que formam junções PN com o canal, conhecidas como regiões de gate. Como pode ser notado na  figura \ref{fig:JFET}. \cite{imd_ufrn_curso_transistores_2025}
...
\section{Transistor Bipolar de Porta Isolada(IGBT)}
        ...
        \section*{Aplicações do IGBT em Circuitos Práticos}

        O IGBT é um componente de alta potência, ideal para aplicações de chaveamento em frequências variando até dezenas de kHz, graças a esse fatores, ele é amplamente utilizado em diversas aplicações industriais e comerciais. A seguir, são descritas algumas das principais aplicações do IGBT:

            \subsection{Inversor de Frequência Monofásico (Meia Ponte ou Ponte Completa)}
                \textbf{Funcionamento:} O inversor utiliza dois IGBTs (Q1 e Q2) em uma configuração de meia ponte. Os IGBTs são chaveados alternadamente por um sinal PWM (Pulse Width Modulation) gerado por um controlador. Quando Q1 está ligado e Q2 desligado, a tensão de saída é positiva. Quando Q2 está ligado e Q1 desligado, a tensão de saída é negativa. A frequência e o ciclo de trabalho do PWM determinam a frequência e a amplitude da tensão AC de saída.

                \begin{figure}[H]
                    \centering
                    \includegraphics[width=0.7\textwidth]{./images/inversorigbt.jpg}
                    \caption{Circuito de inversor de meia ponte com IGBTs.}
                \end{figure}

            \subsection{Conversor Buck (Step-Down) de Alta Potência}
                \textbf{Funcionamento:} O IGBT é chaveado por um sinal PWM(Pulso). Quando está ligado, a corrente flui da fonte de entrada ($V_{in}$) através do indutor (L) e do IGBT para a carga. A energia é armazenada no indutor. Quando IGBT é desligado, o diodo presente entra em condução, permitindo que a energia armazenada no indutor seja entregue à carga, mantendo a corrente. O capacitor (C1) filtra a tensão de saída, resultando em uma tensão DC mais suave e de menor valor.
            ...
            \subsection{Circuito de Controle de um Aquecedor por Indução}
                \textbf{Funcionamento:} O circuito utiliza um IGBT como chave principal para controlar a corrente que flui através de uma bobina de indução. Um oscilador de alta frequência (geralmente na faixa de dezenas a centenas de kHz) gera um sinal PWM que aciona o gate do IGBT. Quando o IGBT liga, a corrente flui pela bobina, criando um campo magnético. Quando o IGBT desliga, o campo magnético colapsa, induzindo uma alta tensão na bobina e gerando correntes parasitas (correntes de Foucault) no objeto metálico colocado próximo à bobina. Essas correntes aquecem o objeto devido à sua resistência. O IGBT é ideal para essa aplicação devido à sua capacidade de chavear altas correntes e tensões em frequências elevadas, com perdas de condução relativamente baixas.

                \begin{figure}[htpb!]
                    \centering
                    \includegraphics[width=0.5\textwidth]{./images/aquecedor_inducao.png}
                    \caption{Circuito de aquecedor por indução com IGBT.}
                \end{figure}
